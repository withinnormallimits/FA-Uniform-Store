...
- **Inventory race**: platforms do not reserve cart stock, so the last unit of a size can be sold twice in the rush; with one-unit-deep sizes this is near-certain. Mitigation: **a written oversell script (apologize + refund or backorder with ETA), and buffer stock on the highest-velocity sizes.** (Lane G)
